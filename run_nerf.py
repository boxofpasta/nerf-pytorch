...
def render_path(render_poses, hwf, chunk, render_kwargs, gt_imgs=None, savedir=None, render_factor=0):
    ...
    for i, c2w in enumerate(tqdm(render_poses)):
        ...
        if gt_imgs is not None and render_factor==0:
            ...
            mse = img2mse(rgb, gt_imgs[i]).cpu().numpy()
            all_mse.append(mse)
    ...
    return rgbs, disps, np.mean(all_mse)
...
def create_nerf(args):
    ...
    # Create optimizer
    optimizer = torch.optim.Adam(params=grad_vars, lr=args.lrate, betas=(args.momentum, args.rms_history))
    print('Using momentum B_1:', args.momentum)
    print('Using rms_prop B_2:', args.rms_history)
...
def config_parser():
    ...
    parser.add_argument("--rms_history", type=float, default=0.99,
                        help='B2 term for Adam')
    ...
    parser.add_argument("--num_poses", type=int, default=1,
                        help='Number of nearest poses used for no_batching = False mode.')
    ...
    parser.add_argument("--N_iters",   type=int, default=200000,
                        help='Number of iterations for this session')
...
def train():
    ...
    if use_batching or K >= len(poses):
        ...
        print('We are taking batches of rays from', K, 'image(s) at a time', flush=True)
        ...
        for i, _ in enumerate(tqdm(train_translations)):
            ...
            distances = np.linalg.norm(train_translations - translation, axis=-1)
            knn_poses_indices = np.array(sorted(train_translations_indices, key=lambda i: distances[i]))[:K]
            knn_poses = train_poses[knn_poses_indices]
            knn_images = train_images[knn_poses_indices]
            nearest_rays.append(get_rays_rgb_for_poses(H, W, knn_poses, knn_images, focal))
    ...
    for i in trange(start, N_iters):
        ...
        select_inds = np.random.choice(rays_rgb.shape[0], size=[N_rand], replace=False)  # (N_rand,)
        # epoch_ended = False
        ...
        adjusted_step = int((global_step - start) * N_rand / float(N_rand_small)) + start
        ...
        if i%args.i_testset==0 and i > 0:
            ...
            with torch.no_grad():
                _, _, test_mse = render_path(torch.Tensor(poses[i_test]).to(device), hwf, args.chunk,
                                             render_kwargs_test, gt_imgs=images[i_test], savedir=testsavedir)
                test_psnr = mse2psnr(torch.from_numpy(np.array(test_mse)))
                writer.add_scalar('mse_test', test_mse, global_step=global_step)
                writer.add_scalar('psnr_test', test_psnr, global_step=global_step)
                writer.add_scalar('psnr_test_by_t', test_psnr, global_step=adjusted_step)
        ...
        if i % args.i_img == 0:
            ...
            writer.add_scalar('mse_holdout_by_t', mse, global_step=adjusted_step)
